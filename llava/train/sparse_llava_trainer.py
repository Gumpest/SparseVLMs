...
class LLaVADynamicTrainer(Trainer):
    def __init__(
        self,
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, 'tune_mm_mlp_adapter', False):
            ...
            super(LLaVADynamicTrainer, self)._save_checkpoint(model, trial, metrics)

    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        if getattr(self.args, 'tune_mm_mlp_adapter', False):
            pass
        else:
            super(LLaVADynamicTrainer, self)._save(output_dir, state_dict)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        with torch.no_grad():
            ...
            # print("teacher", self.teacher_model.device, tea_vision_tower.device)
            teacher_outputs = self.teacher_model(**inputs)
            teacher_hidden_states = teacher_outputs.hidden_states[-1] # last block

        B, L, C = hidden_states.shape
        bool_mask = prev_decision.reshape(B*L) > 0.5
