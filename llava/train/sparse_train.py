...
from llava.train.sparse_llava_trainer import LLaVADynamicTrainer
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        if 'mpt' in model_args.model_name_or_path:
            ...
            tea_model_path = os.path.expanduser(sparse_args.teacher_model_name_or_path)
            tea_model_name = get_model_name_from_path(tea_model_path)
            tea_tokenizer, teacher_model, _, _ = load_pretrained_model(\
                tea_model_path, None, tea_model_name, device_map=None, dynamic_sparse=False)
            print("Teacher initialization")

            teacher_model.eval()
            teacher_model.to(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)
    ...
    if model_args.vision_tower is not None:
        ...
        teacher_model.initialize_vision_tokenizer(model_args, tokenizer=tea_tokenizer)
    ...
    data_module = make_supervised_data_module(tokenizer=tokenizer,
                                              data_args=data_args)
    trainer = LLaVADynamicTrainer(model=model,
                    teacher_model=teacher_model ,  
                    keep_ratio=[0.75, 0.7 ** 2, 0.7 ** 3],
                    tokenizer=tokenizer,
                    args=training_args,
                    **data_module)
