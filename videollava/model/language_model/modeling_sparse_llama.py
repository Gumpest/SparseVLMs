...
class LlamaDynamicvitModel(LlamaModel):
    def __init__(self, config: LlamaConfig, pruning_loc=[1]):
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            if layer_idx in self.pruning_loc and len(pre_prompt_length_list) != 0 and hidden_states.shape[1] !=1:
                ...
                # Inference
                    ...
                    pred_score_vis, s_flag, relation_vis_text = attn_postprocess_rank(attn_logits, v_token_start, v_token_num, text_token_start, t_token_idx) # B, L_v
        ...
        if not self.training:
            ...
            if attention_mask is not None:
                if attention_mask.size() != (bsz, 1, q_len, kv_seq_len):
                    ...
                    # )
                    attention_mask = torch.ones((bsz, q_len)).half().cuda()
                    attention_mask = _prepare_4d_causal_attention_mask(
                        attention_mask, (bsz, q_len), hidden_states, kv_seq_len - past_key_value.get_usable_length(kv_seq_len, self.layer_idx)
                    )
